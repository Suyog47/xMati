...
import { AuthRole, CreateWorkspace, IndustryData, Pipeline, Workspace } from 'common/typings'
...
export class WorkspaceService {
  ...
  async addIndustry(workspaceId: string, industryData: IndustryData): Promise<void> {
    const workspaces = await this.getWorkspaces();
    const workspace = workspaces.find((x) => x.id === workspaceId);

    if (!workspace) {
      throw new Error(`Workspace "${workspaceId}" doesn't exist`);
    }

    if (!workspace.industries) {
      workspace.industries = [];
    }

    const exists = workspace.industries.some(
      (ind) => ind.industry === industryData.industry && ind.subIndustry === industryData.subIndustry
    );

    if (exists) {
      throw new Error(`Industry "${industryData.industry}" with subIndustry "${industryData.subIndustry}" already exists`);
    }

    workspace.industries.push(industryData);

    await this.save(workspaces);
  }

  async getIndustry(workspaceId: string): Promise<IndustryData[]> {
    const workspaces = await this.getWorkspaces();
    const workspace = workspaces.find((x) => x.id === workspaceId);
